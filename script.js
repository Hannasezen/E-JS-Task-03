...
//sort by reviews amount
function arrByReview (arr) {
  return arr.sort(function(obj1, obj2) {
    return parseFloat(obj1.ratingRevievs, 10) - parseFloat(obj2.ratingRevievs, 10);
  })
};

//sort by price
function arrByPrice (arr) {
  for(let obj of arr) {
    if(obj.price.newUan) {
      obj.price = obj.price.newUan;
    }
  };
  return arr.sort(function(obj1, obj2) {
    ...
    return parseFloat(obj1.price.replace(/\s/, ''), 10) - parseFloat(obj2.price.replace(/\s/, ''), 10);
  })
};

console.log(arrByPrice(goods))
//console.log(arrByReview(goods))
